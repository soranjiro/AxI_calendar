...
*   **AWS Lambda (ApiLambda):** Go で実装された単一の関数が、すべての API リクエストのビジネスロジックを実行する。内部で Echo フレームワークを使用し、リクエストパスに基づいて適切なハンドラ処理にルーティングする。認証情報（ユーザーIDなど）は Cognito Authorizer から Lambda のコンテキスト経由で受け取る。
...
*   **ドメイン駆動設計 (Domain-Driven Design - DDD):**
  ...
  *   ビジネスロジックをドメイン層に集約し、インフラストラクチャ（DBアクセス、APIフレームワーク）への依存を低減する。
...
*   API Gateway Cognito Authorizer が `Authorization: Bearer <ID_Token>` を検証し、Lambda コンテキストにユーザー情報 (`sub` = ユーザーID) を渡す。
...
| (代替)エントリ   | `ENTRY#<entry_id>`        | `METADATA`                      | エントリIDで直接取得する場合 (必要に応じて)                       |
...
*   Lambda のエントリーポイント。
*   設定読み込み、DBクライアント (DynamoDB)、認証クライアント (Cognito) の初期化。
...
*   Echo のコンテキストからリクエストパラメータ、ボディ、認証情報 (ユーザーID) を取得。
...
9.  各ステップで発生したエラー (DBエラー、機能実行エラーなど) を適切にハンドリングし、エラーレスポンスを返す。
...
│   ├── api/                # API 関連 (oapi-codegen で生成されるコードを含む)
│   │   ├── handler/        # Echo ハンドラの実装 (e.g., theme_handler.go, entry_handler.go)
│   │   ├── middleware/     # (オプション) カスタムミドルウェア
│   │   ├── router.go       # Echo ルーターの設定
│   │   └── server.go       # oapi-codegen が生成する ServerInterface の実装
│   ├── domain/             # ドメインロジック
...
│   │       ├── monthly_summary.go # 月次集計機能の実装
│   │       ├── task_completion.go # タスク完了率機能の実装
│   │       └── registry.go # 機能レジストリ (featureRegistry)
│   ├── repository/         # データ永続化の実装 (ドメイン層のインターフェースを実装)
│   │   ├── dynamodb/       # DynamoDB 固有の実装
│   │   │   ├── theme_repository.go
│   │   │   ├── entry_repository.go
│   │   │   └── client.go   # DynamoDB クライアント初期化など
│   │   └── mock/           # テスト用のモックリポジトリ (必要に応じて)
│   ├── auth/               # 認証関連 (Cognito クライアント実装)
│   ├── config/             # 設定ファイルの読み込みなど
│   └── infrastructure/     # (オプション) AWS SDK クライアントの共通初期化など
├── api/                    # API 仕様定義
│   └── openapi.yaml
